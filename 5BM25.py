...
for q in xrange(0,len(queries)):
    ...
    for doc in relevantdocs[q]:
        ...
        for i in xrange(len(terms)):
            ...
            df = termctfdf[str(termids[terms[i]])][1]
            doclen = doclengths[doc]
            K = k1*((1.0 - b)+b*(doclen/avgdoclen))
            if doc in terminvlist:
                tf = terminvlist[doc]
                score += math.log(((len(doclengths)-df+0.5)/(df+0.5))*(((k1+1)*tf)/(K+tf)))
        docscores[doc] = score
...
for q in xrange(0,len(queries)):
    ...
    for i in xrange(len(terms)):
        ...
        for doc in relevantdocs[q]:
            ...
            if doc in terminvlist:
                tf = terminvlist[doc]
                docscores[doc] += math.log(((len(doclengths)-df+0.5)/(df+0.5))*(((k1+1)*tf)/(K+tf)))
    queryscores[q] = docscores
